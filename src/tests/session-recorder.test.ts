...
describe("session learning recorder", () => {
	it("builds a privacy-preserving learning record from core events", () => {
		...
		bus.emit({ type: "SESSION_STARTED", sessionId: "session-1", at: "2026-09-13T08:00:00.000Z", goal: "Implement authentication", context: { classId: "class-1", projectId: "project-1", requirementIds: ["requirement-1"], standardIds: ["standard-1"] } });
		...
		expect(record?.session).toMatchObject({ id: "session-1", classId: "class-1", projectId: "project-1", durationSeconds: 1500, goal: "Implement authentication" });
