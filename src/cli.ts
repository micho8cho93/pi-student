...
import { runVozBridge } from "./integrations/paseo/voz-bridge.js";
import { initTheme, InteractiveMode, runRpcMode } from "@earendil-works/pi-coding-agent";
import { createLearningAgentRuntime, createLearningAgentSession } from "./pi/create-session.js";
import { createModelRuntime } from "./pi/model-runtime.js";
import { createRuntimeSessionManager, parseStudentRuntimeArgs, splitModelId } from "./pi/runtime-args.js";
import { runRepl } from "./terminal/repl.js";
import { ensureProviderConfigured, findReadyProvider, SetupCancelledError } from "./terminal/setup.js";
import { redactSecrets } from "./terminal/ui.js";
import { readSandboxMode, SandboxManager } from "./sandbox/sandbox-manager.js";
import { SandboxRuntimeError } from "./sandbox/types.js";
...
import { readSupabaseConfig, saveSupabaseConfig } from "./teacher/config.js";
import { runTeacherCommand } from "./teacher/commands.js";
...
async function runTerminal(flags: string[]): Promise<void> {
	...
	startupCues.start();
	const sandboxManager = new SandboxManager(cwd);
	try {
	...
	const workflow = new WorkflowController(createLearningSession(cwd));
		if (interactive) {
			terminal.close();
			const agent = await createLearningAgentRuntime(cwd, workflow, modelRuntime, model, sandboxManager.runtime);
		...
		const agent = await createLearningAgentSession(cwd, workflow, modelRuntime, model, sandboxManager.runtime);
		startupCues.stop("Pi Student is ready");
		try {
...
async function runSharedRuntime(flags: string[]): Promise<void> {
	...
	try {
		...
		if (!handedToRpc) await sandboxManager.stop();
	}
}
