...
import { updateProjectCapabilities } from "./class-service.js";
import process from "node:process";
import { createInterface, type Interface } from "node:readline/promises";
import type { Readable, Writable } from "node:stream";
import type { SupabaseClient } from "@supabase/supabase-js";
import { createTheme, renderPanel, type PanelRow, type TerminalTheme } from "../terminal/ui.js";
import { createPiSupabaseClient } from "./auth.js";
import type { SupabaseConfig } from "./config.js";
import { describeAuthError } from "./auth-flow.js";
import { openBrowser } from "./open-browser.js";
import { DictationController } from "../voice/dictation.js";
import { runDictationSetup, type DictationSetupUI } from "../voice/dictation-setup.js";
import { execFileSync } from "node:child_process";
import { createDefaultProjectBuilder, formatProjectBrief, normalizeProjectBrief, type ProjectBrief, type ProjectBuilderMessage } from "./project-builder.js";
import { saveProjectDraft, updateProjectBrief } from "./class-service.js";
...
async function handleInput(client: SupabaseClient, state: { screen: Screen; choices: Array<{ type: string; [key: string]: unknown }> }, command: string, args: string[], readline: Interface, theme: TerminalTheme, write: (value: string) => void) {
	...
	if (state.screen.tab === "projects" && command === "controls") {
		...
		if (policy) await updateProjectCapabilities(client, project.id, policy);
		return;
	}
...
async function createProject(client: SupabaseClient, item: TeacherClass, readline: Interface, write: (value: string) => void, theme: TerminalTheme) {
	...
	while (true) {
		...
		if (choice === "" || choice === "y" || choice === "yes") {
			const saved = await saveProjectDraft(client, item.id, turn.draft);
...
async function editProject(client: SupabaseClient, choices: Array<{ type: string; [key: string]: unknown }>, value: string | undefined, readline: Interface) {
	...
	await updateProjectBrief(client, project.id, { name: name || project.name, description: description || null, brief: { version: 1, goal, objectives, expectations, structure, constraints, successCriteria } });
}
