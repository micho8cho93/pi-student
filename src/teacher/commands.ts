...
import { joinClass } from "./class-service.js";
import { readSupabaseConfig } from "./config.js";
import { startTeacherDashboard } from "./dashboard-server.js";
import { LearningRecordStore, readTeacherContext, writeTeacherContext } from "../telemetry/local-store.js";
import { createSupabaseUploader, LearningRecordSyncService } from "../telemetry/sync-service.js";
import { openBrowser } from "./open-browser.js";
import { describeAuthError, teacherAuthCallbackUrl } from "./auth-flow.js";
...
export async function runTeacherCommand(args: string[]): Promise<boolean> {
	...
	if (area === "class") {
		...
		if (action === "join") {
			...
			const membership = await joinClass(client, code);
	...
	if (area === "sync") {
		const client = createPiSupabaseClient(requireConfig());
		const result = await new LearningRecordSyncService(new LearningRecordStore(), createSupabaseUploader(client)).syncPending();
...
export async function authenticateInBrowser(client: SupabaseClient, method: "email" | "google", email?: string, notify: (message: string) => void = message => process.stdout.write(`${message}\n`)): Promise<void> {
	...
	const server = createServer(async (request, response) => {
		try {
			...
			response.end("Pi Student sign-in complete. You can close this tab.");
			resolveAuth();
		} catch (error) {
			response.writeHead(400, { "content-type": "text/plain; charset=utf-8" });
			response.end("Pi Student could not complete sign-in.");
	...
	try {
		if (method === "google") {
			...
			const { error } = await client.auth.signInWithOtp({ email: email!, options: { emailRedirectTo: redirectTo } });
			if (error) throw new Error(describeAuthError(error));
			notify(`Open the magic link sent to ${email}. Waiting for sign-in…`);
		}
		await Promise.race([
			completed,
			new Promise<never>((_, reject) => { timeout = setTimeout(() => reject(new Error("Sign-in timed out after ten minutes.")), 10 * 60 * 1000); }),
		]);
