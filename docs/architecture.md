...
# Phase 4 organization sandbox and extension control plane

The organization administrator's Sandbox section stores a domain block list for Gondolin HTTP/HTTPS requests. `resolve_institutional_environment` authorizes the active class member and returns that list, any assigned profile, and enabled, approved, in-scope Skill/MCP descriptors; it never returns secret references or endpoint/command metadata. Gondolin denies each listed hostname and its subdomains, and its HTTP hooks also keep the default internal-address checks. This is domain filtering for the HTTP/HTTPS proxy path, not category filtering or a complete network firewall. The runtime combines this with `EffectivePolicy` before sandbox startup. For personal and standalone classroom sessions, the existing local sandbox remains in use.

The profile and dataset registries and provider-neutral contracts remain in the database, but the org admin no longer exposes their controls. The packaged Gondolin provider still cannot consume a verified custom image or artifact-mounted dataset and intentionally rejects managed profiles. Skills remain disabled in Pi's student resource loader and MCP registry entries remain non-executable metadata until a verified extension broker enforces capabilities and host-managed secrets. See ADRs 0012–0014 for the trust boundary and outstanding provider/broker work.
