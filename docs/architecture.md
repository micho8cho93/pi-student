...
packages/
  ...
  education/ policy/ classroom/ telemetry/ publishing/ sandbox/
...
`tooling/check-boundaries.mjs` fails CI for package-to-application imports, core-to-concrete-adapter imports, Supabase imports in classroom/education/policy, runtime imports of Gondolin/Paseo/teacher UI, contracts imports of other Pi Student packages, and `src` deep imports across workspace packages. `npm run test:architecture` also verifies the public package roots.
...
Organizations are intentionally absent. A future control plane can implement `IdentityProvider`, `PolicyProvider`, `ModelProvider`, `ClassroomRepository`, `TelemetrySink`, `SkillProvider`, and `McpProvider`, then supply those implementations to `createStudentRuntime`. The control plane can translate platform → organization → class → project → session configuration into one `RuntimeConfiguration`; the learning engine does not need organization administration, tenancy, billing, or database concepts.

See [the Organization-readiness audit](organization-readiness.md) for current role, ownership, policy, telemetry, model, and sandbox assumptions. Decisions are recorded in [the ADR index](adr/README.md).
