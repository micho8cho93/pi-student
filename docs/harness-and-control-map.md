...
| Project controls | A policy changes, is unavailable, or a session cap is reached mid-turn | Managed policy resolution fails closed; local project policy may be cached offline. Token/cost limits are evaluated after a response, so that response can exceed the cap. |
