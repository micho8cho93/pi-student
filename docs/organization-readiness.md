...
The teacher console still creates and lists standalone classes by `teacher_id`. Class conversion, email invitations, billing, LMS integrations, sandbox profiles, Skills/MCP administration, private model endpoints, enterprise SSO, and support impersonation remain deferred. Any future support access must be time-limited, explicit, and audited.
