...
async function ensureEcosystemBridge(paths: InstallationPaths, port: number, runtimeEntry?: string): Promise<void> {
	if (await isEcosystemBridgeRunning(port)) {
		...
		const health = await response.json() as { learnMode?: boolean };
		if (health.learnMode !== true) throw new Error("An older Pi Student background bridge is still running. Stop the existing pi-student ecosystem-bridge process and relaunch the GUI to enable Learn Mode.");
		return;
	}
	const entry = await findRuntimeEntry(runtimeEntry);
