...
describe("project capability controls", () => {
	...
	it("accepts policies saved before the tutoring reserve existed", () => { const { tutoringTurns: _, ...limits } = policy().limits; expect(parseCapabilityPolicy({ ...policy(), limits }).limits.tutoringTurns).toBeNull(); expect(() => parseCapabilityPolicy({ ...policy(), limits: { ...limits, tutoringTurns: 0 } })).toThrow(/tutoringTurns/); });
