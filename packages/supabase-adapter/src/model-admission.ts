...
export class SupabaseModelAdmissionProvider implements ModelAdmissionProvider {
	constructor(private readonly client: SupabaseClient, private readonly refreshGatewayToken?: (token: string, sessionId?: string) => void) {}
	async check(projectId: string, provider: string, modelId: string, thinkingLevel: ThinkingLevel, sessionId?: string) {
		...
		this.refreshGatewayToken?.(auth.session.access_token, sessionId);
