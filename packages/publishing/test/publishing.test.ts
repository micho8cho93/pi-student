...
import { detectProjectType } from "@pi-student/publishing/project-detection";
import { GitRunner, type CommandExecutor } from "@pi-student/publishing/git-runner";
...
class FakeGitHub implements GitHubClient {
	...
	removed: Array<{ owner: string; repo: string }> = [];
	...
	async removePages(owner: string, repoName: string) { this.removed.push({ owner, repo: repoName }); }
...
describe("publishing service", () => {
	it("removes GitHub Pages without removing repository metadata and can publish again", async () => {
		...
		try {
			...
			await service.removeSite(f.project);
			expect(github.removed).toEqual([{ owner: "student", repo: "my-portfolio" }]);
			const removed = await f.metadata.read(f.project);
			expect(removed?.deployment).toBeUndefined();
			expect(removed?.github).toMatchObject({ owner: "student", repo: "my-portfolio" });
			await service.publish(f.project);
			expect(github.repositories.size).toBe(1);
			expect(github.configured).toHaveLength(2);
...
describe("GitHub API adapter", () => {
	it("deletes only the GitHub Pages site", async () => {
		const calls: string[][] = [];
		const client = new GhGitHubClient(async args => { calls.push(args); return ""; });
		await client.removePages("student", "site");
		expect(calls).toContainEqual(expect.arrayContaining(["api", "/repos/student/site/pages", "--method", "DELETE"]));
	});
