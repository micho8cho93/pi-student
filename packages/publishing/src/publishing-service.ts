...
import type { DeploymentProvider, GitHubClient, ProjectPublishingMetadata, PublishProgress, PublishResult, RepositorySummary } from "./types.js";
...
export class PublishingService {
	...
	async publish(requestedPath: string): Promise<PublishResult> {
		...
		try {
			...
			else await this.provider.deploy(deploymentInput);
	...
	async removeSite(requestedPath: string): Promise<void> {
		const projectPath = await realpath(requestedPath);
		const metadata = await this.metadata.read(projectPath);
		if (!metadata?.github || !metadata.deployment) throw new Error("This project does not have a published site to remove.");
		const connection = await this.options.github.getConnection();
		if (!connection.connected) throw new Error("GitHub must be connected before removing this site.");
		await this.provider.remove(metadata.github.owner, metadata.github.repo);
		await this.metadata.write(projectPath, { ...metadata, deployment: undefined });
	}
