...
export class GhGitHubClient implements GitHubClient {
	...
	async removePages(owner: string, repo: string): Promise<void> {
		try {
			await this.api("DELETE", `repos/${segment(owner)}/${segment(repo)}/pages`);
		} catch (error) {
			// Treat an already-removed Pages site as success so Pi can repair stale local state.
			if (!isNotFound(error)) throw new Error(friendlyGitHubError(error));
		}
	}
