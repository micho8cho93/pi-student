...
export interface TeacherContextStore {
	read(): Promise<TeacherContext>;
	write(context: TeacherContext): Promise<void>;
}

export class FileTeacherContextStore implements TeacherContextStore {
	constructor(private readonly filePath = path.join(getInstallationPaths().config, "teacher-context.json")) {}
	read(): Promise<TeacherContext> { return readTeacherContext(this.filePath); }
	write(context: TeacherContext): Promise<void> { return writeTeacherContext(context, this.filePath); }
}
