...
async function loadTeacherExtensions(kind){
  ...
  for(const item of rows){
    const blocked=projectId?item.projectBlocked:item.classBlocked;
    const inherited=!!projectId&&item.classBlocked;
    const input=node('input',{type:'checkbox','aria-label':'Allow '+item.name+' for '+(projectId?'project':'class')});
    input.checked=!blocked&&!inherited;input.disabled=inherited;
    input.onchange=async()=>{
      input.disabled=true;
      const {error:saveError}=await db.rpc('set_teacher_extension_enabled',{
        class_id_input:classId,project_id_input:projectId,kind_input:item.kind,extension_id_input:item.id,enabled_input:input.checked
      });
      settingsNote(kind+'-note',saveError?errorMessage(saveError):'Availability saved.',!!saveError);
      if(saveError){input.checked=!blocked&&!inherited;input.disabled=false;return}
      await loadTeacherExtensions(kind);
    };
    const description=item.description||((item.capabilities||[]).length?'Capabilities: '+item.capabilities.join(', '):'Approved by your school');
    list.append(node('label',{class:'extension-row'},[node('span',{},[node('strong',{},item.name+' · '+item.version),node('small',{},description),node('small',{class:'muted'},inherited?'Blocked for this class':item.scope+' scope')]),input]));
  }
  root.replaceChildren(node('div',{class:'panel'},[node('div',{class:'panel-head'},[node('div',{},[node('h2',{},projectId?'Project availability':'Class availability'),node('p',{},'Class restrictions also apply to every project in the class.')])]),list]));
}
