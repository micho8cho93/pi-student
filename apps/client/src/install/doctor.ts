...
import { isSandboxImageAvailable, runSandboxHealthCheck } from "@pi-student/sandbox-gondolin/health-check";
import { normalizePlatform } from "@pi-student/sandbox-gondolin/platform";
import { readRuntimeMetadata } from "@pi-student/sandbox-gondolin/runtime-metadata";
import { findExecutable, resolveSandboxRuntime } from "@pi-student/sandbox-gondolin/runtime";
import { readAndValidatePaseoConfig } from "@pi-student/paseo-adapter/config";
...
export async function runDoctor(options: { verbose?: boolean; cwd?: string } = {}): Promise<boolean> {
	...
	application.push(await fileCheck("Pi Student installed", path.join(paths.app, "apps", "client", "dist", "cli.js"), options.verbose));
