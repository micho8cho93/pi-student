...
The gateway supports text-only OpenAI-compatible chat completions. It reserves a maximum of 32,000 input and 4,096 output tokens, then settles provider-reported usage before returning a response. An upstream or settlement failure leaves a reservation held; review those rows operationally before adjusting budgets or reclaiming capacity. A missing price blocks requests under a cost budget. The gateway should be protected by normal host and network controls in addition to its per-request authorization.
